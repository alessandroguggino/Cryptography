...
# check if they are ASCII chars
# if all the decryptions are ASCII then ks_byte_candidate is a real candidate
# else drop them

#1
print('>>> Guess first byte of KS')
...
print(chr(ciphertexts[0][0] ^ 0)) # ASCII upper case
...
print(chr(ciphertexts[0][0] ^ 32)) # ASCII lower case

# save all the candidate values that reached the maximum score
m = max(counter)
maxm = []
for i in range(256):
    if counter[i] == m:
...
candidate_key_byte = []
for pos in range(1, max_len): # exclude first byte
    counter = numpy.zeros(256, dtype=int)
    for key_byte in range(256):
        for c in ciphertexts:
            if pos < len(c): # because some ciphertexts are shorter than max_len
...
print("Estimate the cost of a brute force attack:", end=' ')
...
print('--- Keystream found ---')
print(keystream)

# here we have a first plaintext
print('--- Plaintext ---')
for c in ciphertexts:
    print(bytes_xor(c, keystream))

# correct the plaintext modifying the keystream
print(">>> Manual correction of the keystream")
new_keystream = bytearray()
new_keystream += bytes([ord(' ') ^ ord('T') ^ keystream[0]]) # instead of space put T
new_keystream += bytes([ord(' ') ^ ord('h') ^ keystream[1]]) # instead of space put h
new_keystream += bytes([ord(',') ^ ord('i') ^ keystream[2]]) # instead of , put i
