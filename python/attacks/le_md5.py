...
if __name__ == "__main__":
    ...
    message = b'ciao' # public (we are considering integrity not confidentiality)
    to_add = b'ciao'

    # keyed-digest = (k || message) --> MAC = m
    # starting from m we build keyed-digest = (message || additional_data) without knowing k
    m = md5()
    m.update(secret+message)
    print(m.hexdigest())

    digest = m.digest()

    print("state: " + str(m.state))
    print("encoding state: " + str(_encode(m.state, md5.digest_size)))

    pad = padding(len(secret+message) * 8)
    print("pad = " + str(pad))

    # digest = (secret || message) + padding
    # pass the digest as state
    m2 = md5(state=digest, count=512)
    m2.update(to_add) # add additional_data
    print(m2.hexdigest())

    # check that the code performs the attack
    m3 = md5()
    m3.update(secret + message + pad + to_add)
    print(m3.hexdigest())

    # m2.digest == m3.digest --> OK

    # intercept message and digest
    # new message = message + padding(len(secret+message) * 8) + to_add
    # new digest = m2.hexdigest()
